<?php
	...
	$sql = "SELECT s.*, v.*, date_format(v.data_inclusao, '%d %b %Y %T') AS data_inclusao_formatada, e.nome_fantasia FROM vagas AS v LEFT JOIN usuarios_vagas AS s ON (s.id_usuario = 'id_usuario' AND v.id_vaga = s.vinculado_id_vaga) JOIN empresas AS e ON (v.id_empresa = e.id_empresa)";
